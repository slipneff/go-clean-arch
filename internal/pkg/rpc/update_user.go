...
func (s *Server) UpdateUser(ctx context.Context, req *desc.UpdateUserRequest) (*desc.UserResponse, error) {
	log.Debug(fmt.Sprintf("UpdateUser Request, %s", json.ToColorJson(req)))
	if req == nil {
		return &desc.UserResponse{Error: desc.UserErrorCode_USER_ERROR_CODE_VALIDATION}, nil
	}
	var userId uuid.UUID
	if validateUUID(req.User.UserId, &userId) != nil {
		return &desc.UserResponse{Error: desc.UserErrorCode_USER_ERROR_CODE_VALIDATION}, nil
	}

	response, err := s.userService.UpdateUser(ctx, model.User{Id: userId, Name: req.User.Name})
	if err != nil {
		if err == sql.ErrEntityExist {
			return &desc.UserResponse{Error: desc.UserErrorCode_USER_ERROR_CODE_ALREADY_EXIST}, nil
		}
		log.Error(err, "Error in service UpdateUser", "[RPC] UpdateUser")
		return nil, err
	}
